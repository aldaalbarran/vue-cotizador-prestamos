...
<template>
  <div class="my-20 max-w-lg mx-auto bg-white shadow p-10">
    ...
    <div class="my-5">
      <input type="range" name="" id="" class="w-full bg-gray-200 accent-violet-500 hover:accent-violet-600" :min="MIN" :max="MAX" :step="STEP" v-model.number="cantidad">
      <p class="text-center my-3 text-5xl font-bold text-indigo-600">{{ formatearDinero(cantidad) }}</p>
      <h2 class="text-2xl font-extrabold text-gray-500 text-center">Elige un <span class="text-indigo-600">plazo</span> a pagar</h2>
      <select name="" id="" class="w-full p-2 mt-5 bg-white border border-gray-300 rounded-lg text-center text-xl font-bold text-gray-500" :value="meses" v-model.number="meses">
    ...
    <p v-else class="text-center text-gray-600">Añade una cantidad y un plazo a pagar.</p>
  </div>
</template>
